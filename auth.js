document.addEventListener('DOMContentLoaded', () => {
    ...
    // Přidání naslouchání na custom event 'openAuth'
    document.addEventListener('openAuth', (e) => {
        const tab = e.detail && e.detail.tab ? e.detail.tab : 'login';
        openModal(tab);
    });

    // --- SESSION PERSISTENCE ---
    const checkSession = () => {
        const savedUser = localStorage.getItem('mates_logged_in');
        if (savedUser) {
            const user = JSON.parse(savedUser);
            loginUser(user, false); // false = don't alert
        }
    };

    const loginUser = (user, showAlert = true) => {
        if (showAlert) alert(`Vítej zpět, ${user.username}!`);
        closeModal();
        window.currentUser = user; // Make it global
        localStorage.setItem('mates_logged_in', JSON.stringify(user));

        // Upravíme navigaci - McRybar Style
        document.querySelectorAll('.nav-right').forEach(navRight => {
            navRight.innerHTML = `
                <div class="nav-profile-wrapper" id="navProfile">
                    <img src="https://mc-heads.net/avatar/${user.username}/100" class="nav-avatar">
                    <span class="nav-nick">${user.username}</span>
                    <div class="profile-dropdown" id="profileDropdown">
                        <div class="dropdown-header">
                            <div class="dropdown-user-info">
                                <img src="https://mc-heads.net/avatar/${user.username}/100" class="dropdown-avatar-large">
                        ...
                        <div class="dropdown-section">
                            <span class="dropdown-label">Account Info</span>
                            <div class="dropdown-item" style="cursor: default; justify-content: space-between;">
                                <span>Heslo:</span>
                                <span class="view-pass-masked" style="filter: blur(4px); transition: 0.3s; cursor: pointer;" onclick="event.stopPropagation(); this.style.filter = this.style.filter ? '' : 'blur(4px)'">${user.password}</span>
                            </div>
                        </div>

                        <div class="dropdown-section">
                            <span class="dropdown-label">Tickety</span>
                            <a href="tickets.html" class="dropdown-item">🎫 Moje Tickety</a>
                            <a href="tickets.html?action=new" class="dropdown-item">🎫 Vytvořit Ticket</a>
                        </div>

                        ${user.isAdmin ? `
                        <div class="dropdown-section">
                            <span class="dropdown-label">Support Panel</span>
                            <a href="tickets.html?view=staff" class="dropdown-item staff">🛡️ Seznam Ticketů</a>
                            <a href="tickets.html?view=closed" class="dropdown-item">📁 Uzavřené</a>
                        </div>
                        ` : ''}

                        <a href="#" class="dropdown-item logout" onclick="window.logout()">🚪 Odhlásit se</a>
                    </div>
                </div>
                <a href="kontakt.html" class="nav-item">✉️ Kontakt</a>
            `;
            
            const profileTrigger = navRight.querySelector('#navProfile');
            if (profileTrigger) {
                profileTrigger.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const dropdown = profileTrigger.querySelector('#profileDropdown');
                    dropdown.classList.toggle('active');
                });
            }
        });
        
        // Zavření dropdownu kliknutím jinam
        document.addEventListener('click', () => {
            document.querySelectorAll('.profile-dropdown').forEach(d => d.classList.remove('active'));
        });

        // Dispatch event for other scripts (like tickets.html)
    ...
    window.logout = () => {
        localStorage.removeItem('mates_logged_in');
    ...
    // 4. Logika Přihlášení (Stará logika odstraněna a sjednocena výše)
    ...
    // --- SYSTÉM RECENZÍ (Nové) ---
    // window.currentUser is used instead
    ...
    function renderReviews() {
        ...
        container.innerHTML = reviews.map((rev, index) => {
            ...
            const isAdmin = user && (user.username === 'Mates' || user.isAdmin || user.email === 'smejkalsmejki@gmail.com');
    ...
    const songs = [
        ...
        { 
            title: 'No Signal', 
            artist: 'Eyedress', 
            url: 'assets/music/no signal (Ultra Slowed).mp3',
            cover: 'assets/music/eyedress.png'
        },
        { 
            title: 'New Jeans Jersey Remix (Part 2)', 
            artist: 'Jiandro', 
            url: 'assets/music/𝘑𝘪𝘢𝘯𝘥𝘳𝘰 - 𝘕𝘦𝘸 𝘑𝘦𝘢𝘯𝘴 𝘑𝘦𝘳𝘴𝘦𝘺 𝘙𝘦𝘮𝘪𝘹 ( 𝘗𝘢𝘳𝘵 2 ) - 𝘚𝘭𝘰𝘸𝘦𝘥 𝘝𝘦𝘳𝘴𝘪𝘰𝘯.mp3',
            cover: 'assets/music/gata_only.png'
        },
        { 
            title: 'Kehlani (Slowed Reverb)', 
            artist: 'Jordan Adetunji', 
            url: 'assets/music/jordan adetunji - kehlani [ slowed  reverb ] (lyrics).mp3',
            cover: 'assets/music/eyedress.png'
        },
        { 
            title: 'Rock', 
            artist: 'Stepz', 
            url: 'assets/music/Stepz - Rock.mp3',
            cover: 'assets/music/gata_only.png'
        },
        { 
            title: 'Big Guy', 
            artist: 'Ice Spice', 
            url: 'assets/music/Ice Spice - Big Guy (Lyrics).mp3',
            cover: 'assets/music/gata_only.png'
        },
        { 
            title: 'HEAVENLY JUMPSTYLE', 
            artist: 'Unknown', 
            url: 'assets/music/HEAVENLY JUMPSTYLE.mp3',
            cover: 'assets/music/eyedress.png'
        }
    ];
        ...
        const musicHTML = `
            <div class="music-widget">
                ...
                <div class="music-progress-container">
                    <input type="range" class="music-progress-bar" id="progressBar" min="0" max="100" value="0" step="0.1">
    ...
    // Hook do přihlášení pro aktualizaci UI recenzí (Naskládáno do loginUser)
});
